  ...
  bool bridge_all_1to2_topics, bool bridge_all_2to1_topics)
{
  ...
  for (auto ros2_publisher : ros2_publishers) {
    ...
    try {
      std::cout << "ros1_type_name " << bridge.ros1_type_name << std::endl;
      std::cout << "ros2_type_name " << bridge.ros2_type_name << std::endl;
    ...
    ](const ros::TimerEvent &) -> void
    {
      ...
      for (auto topic : topics) {
        ...
        if (!has_publisher && !has_subscriber) {
          ...
          std::cout << "has_publisher: "<<has_publisher<<std::endl;
          std::cout << "has_subscriber: "<<has_subscriber<<std::endl;
          continue;
        }
        if (has_publisher) {
